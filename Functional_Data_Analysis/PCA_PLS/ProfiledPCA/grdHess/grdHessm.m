function [grd, Hess] = grdHessm(f, x, h0, toler, ymat, lambda, Rmat) 
%Input
%       - f is the function
...
for i = 1:p
    if nargout == 2
        [grdi, dHessi] = grdHessD(f,x,f0,i,h0,toler,ymat,lambda,Rmat);
        grd(:,i)   = grdi;
        dHess(:,i) = dHessi;
    else
        grd(:,i) = grdHessD(f,x,f0,i,h0,toler,ymat,lambda,Rmat);
    end
end
...
function [grdi, dHessi] = grdHessD(f,x,f0,idx,h0,toler,ymat,lambda,Rmat)
eps = 2.220446e-16;
p   = length(x);
n   = length(f0);
h   = h0;
...
hp      = zeros(p,1);
hp(idx) = h;
fh      = f(x+hp,ymat,lambda,Rmat);
fmh     = f(x-hp,ymat,lambda,Rmat);
if nargout == 2
    Dmat1 = zeros(n,12,12);
    Dmat2 = Dmat1;
...
function Hessik = HessOD(f,x,f0,idx1,idx2,h0,toler,ymat,lambda,Rmat)
eps = 2.220446e-16;
p   = length(x);
n   = length(f0);
h   = h0;
...
Dmat = zeros(n,12,12);
Dmat(:,1,1) = ...
    (f(x+hp1+hp2,ymat,lambda,Rmat) + ...
     f(x-hp1-hp2,ymat,lambda,Rmat) - ...
     f(x-hp1+hp2,ymat,lambda,Rmat) - ...
     f(x+hp1-hp2,ymat,lambda,Rmat))/(4*h^2);
rerr = ones(n,1);
