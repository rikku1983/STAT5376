function [grd, Hess] = ...
    grdHessm_FDA(f, x, h0, toler, Xfd, AfdPar, gamval, Ymat, ...
                 PenStruct, radius, anglewrd, GrssMnWrd, GrssMnMap) 
%Input
%       - f is the function
...
f0 = f(x,Xfd, AfdPar, gamval, Ymat, PenStruct, ...
...
for i = 1:p
    if nargout == 2
        [grdi, dHessi] = grdHessD(f,x,f0,i,h0,toler,Xfd, AfdPar, ...
                                  gamval, Ymat, PenStruct, ...
                                  radius, anglewrd, GrssMnWrd, GrssMnMap);
        grd(:,i)   = grdi;
        dHess(:,i) = dHessi;
    else
        grd(:,i) = grdHessD(f,x,f0,i,h0,toler,Xfd, AfdPar, ...
                            gamval, Ymat, PenStruct, ...
                            radius, anglewrd, GrssMnWrd, GrssMnMap);
    end
end
...
function [grdi, dHessi] = grdHessD(f,x,f0,idx,h0,toler,Xfd, AfdPar, ...
                                   gamval, Ymat, PenStruct, ...
                                   radius, anglewrd, GrssMnWrd, GrssMnMap)
eps = 2.220446e-16;
p   = length(x);
...
hp      = zeros(p,1);
hp(idx) = h;
fh      = f(x+hp,Xfd, AfdPar, gamval, Ymat, ...
            PenStruct, radius, anglewrd, GrssMnWrd, GrssMnMap);
fmh     = f(x-hp,Xfd, AfdPar, gamval, Ymat, ...
            PenStruct, radius, anglewrd, GrssMnWrd, GrssMnMap);
if nargout == 2
    Dmat1 = zeros(n,12,12);
    Dmat2 = Dmat1;
...
function Hessik = HessOD(f,x,f0,idx1,idx2,h0,toler,Xfd, AfdPar, ...
                         gamval, Ymat, PenStruct, ...
                         radius, anglewrd, GrssMnWrd, GrssMnMap)
eps = 2.220446e-16;
p   = length(x);
...
Dmat = zeros(n,12,12);
Dmat(:,1,1) = ...
    (f(x+hp1+hp2,Xfd, AfdPar, gamval, Ymat, ...
       PenStruct, radius, anglewrd, GrssMnWrd, GrssMnMap) + ...
     f(x-hp1-hp2,Xfd, AfdPar, gamval, Ymat, ...
       PenStruct, radius, anglewrd, GrssMnWrd, GrssMnMap) - ...
     f(x-hp1+hp2,Xfd, AfdPar, gamval, Ymat, ...
       PenStruct, radius, anglewrd, GrssMnWrd, GrssMnMap) - ...
     f(x+hp1-hp2,Xfd, AfdPar, gamval, Ymat, ...
       PenStruct, radius, anglewrd, GrssMnWrd, GrssMnMap))/(4*h^2);
rerr = ones(n,1);
