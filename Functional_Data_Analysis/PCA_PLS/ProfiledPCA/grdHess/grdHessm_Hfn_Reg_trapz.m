function [grd, Hess] = grdHessm_Hfn_Reg_trapz(f, x, h0, toler, tfine, Ymat, ...
...
f0 = f(x, tfine, Ymat, AfdPar, WfdPar, PenStruct, Joptions);
p  = length(x);
n  = length(f0);
...
for i = 1:p
    if nargout == 2
        [grdi, dHessi] = grdHessD(f,x,f0,i,h0,toler, tfine, Ymat, ... 
                                  AfdPar, WfdPar, PenStruct, Joptions);
        grd(:,i)   = grdi;
        dHess(:,i) = dHessi;
    else
        grd(:,i) = grdHessD(f,x,f0,i,h0,toler, tfine, Ymat, ...  
                             AfdPar, WfdPar, PenStruct, Joptions);
    end
end
...
if nargout == 2
    ...
    for i = 1:lpi
        ...
        idx2 = idx(i,2);
        Hess(:,idx1,idx2) = ...
            HessOD(f,x,f0,idx1,idx2,h0,toler, tfine, Ymat, ... 
                   AfdPar, WfdPar, PenStruct, Joptions);
...
function [grdi, dHessi] = grdHessD(f,x,f0,idx,h0,toler, tfine, Ymat, ...
                                   AfdPar, WfdPar, PenStruct, Joptions)
eps = 2.220446e-16;
p   = length(x);
n   = length(f0);
...
hp      = zeros(p,1);
hp(idx) = h;
fh      = f(x+hp, tfine, Ymat, AfdPar, WfdPar, PenStruct, Joptions);
fmh     = f(x-hp, tfine, Ymat, AfdPar, WfdPar, PenStruct, Joptions);
if nargout == 2
    Dmat1 = zeros(n,12,12);
    Dmat2 = Dmat1;
...
function Hessik = HessOD(f,x,f0,idx1,idx2,h0,toler, tfine, Ymat, ...
                         AfdPar, WfdPar, PenStruct, Joptions)
eps = 2.220446e-16;
p   = length(x);
n   = length(f0);
...
Dmat = zeros(n,12,12);
Dmat(:,1,1) = ...
    (f(x+hp1+hp2, tfine, Ymat, AfdPar, WfdPar, PenStruct, Joptions) + ...
     f(x-hp1-hp2, tfine, Ymat, AfdPar, WfdPar, PenStruct, Joptions) - ...
     f(x-hp1+hp2, tfine, Ymat, AfdPar, WfdPar, PenStruct, Joptions) - ...
     f(x+hp1-hp2, tfine, Ymat, AfdPar, WfdPar, PenStruct, Joptions))/(4*h^2);
rerr = ones(n,1);
